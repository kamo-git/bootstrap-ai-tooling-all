...
## 9. テストの義務
- **コーディング開始前**に、必ず以下を実施することを義務とする：
  - **影響範囲の調査**：変更が影響する範囲（ファイル、関数、DB、API、画面等）を特定し、列挙する。
  - **テストの作成**：変更内容を検証するためのテスト（手動テスト手順書または自動テストコード）を作成する。
  - **テストの実施（変更前ベースライン）**：既存コードに対してテストを実施し、現在の動作を確認する。変更前の動作をベースラインとして記録する。
- **コード修正・新規実装後**は、必ず以下を実施することを義務とする：
  - **テストの再実施**：コーディング前に作成したテストを再実行し、変更後も正しく動作することを確認する。
  - テストなしでのコミット・PR作成は禁止。
...
- **テスト実施のワークフロー（必須）**：
  1. 影響範囲を調査し、文書化する。
  2. テストケースを作成する（手動手順書または自動テストコード）。
  3. 変更前のコードに対してテストを実施し、ベースラインを確認する。
  4. コードを変更する。
  5. 変更後のコードに対してテストを再実施し、期待通り動作することを確認する。
  6. PR説明に「影響範囲」「テスト内容」「テスト結果（変更前/変更後）」を明記する。
- テスト省略が許されるケース：
  - ドキュメントのみの変更（コード影響なし）。
...
## 10. コード削除の方針
- コード削除時は、必ず削除の意図を確認し、慎重に実施する。
  - 削除理由をPR説明またはコミットメッセージに明記する。
  - 削除したコード（関数、クラス、メソッド、APIエンドポイント、設定項目等）を明示する。
  - 削除による影響範囲（依存コード、呼び出し元、関連ドキュメント等）を検討し、必要に応じて更新する。
- 意図しない削除は厳禁。以下を特に注意する：
  - リファクタリング時の機能削除。
  - コピー＆ペースト時の元コード削除。
  - 自動生成ツールによる既存コードの上書き。
- 特に慎重に扱うべき削除：
  - 公開API、エンドポイントの削除（互換性に影響）。
  - データベーススキーマの削除（データ損失リスク）。
  - 設定ファイル、環境変数の削除（運用影響）。
  - エラーハンドリング、バリデーションの削除（品質劣化）。
